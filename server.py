...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'color_recognition'))

from color_recognition.color_cnn import ColorCNN
...
class CNNPredictor(ColorPredictor):
    ...
    def predict_pixel(self, patch):
        ...
        return self.label_mapping.get(prediction_idx, f"class_{prediction_idx}")
...
def run(server_class=HTTPServer, handler_class=SimpleJSONHandler, port=8000, predictor=None):
    server_address = ("", port)
    httpd = server_class(server_address, handler_class)
    httpd.predictor = predictor
    print(f"Server running on http://localhost:{port} with {predictor.get_name()} model")
    httpd.serve_forever()
...
def get_cube_corners(edges):
    ...
    alpha_1 = math.acos((side_1**2 + side_2**2 - diag_1**2) / (2 * side_1 * side_2))
    alpha_2 = math.acos((side_2**2 + side_3**2 - diag_2**2) / (2 * side_2 * side_3))
    alpha_3 = math.acos((side_3**2 + side_4**2 - diag_1**2) / (2 * side_3 * side_4))
    alpha_4 = math.acos((side_4**2 + side_1**2 - diag_2**2) / (2 * side_4 * side_1))
    
    area = (side_1 * side_2 * math.sin(alpha_1) / 2) + (side_3 * side_4 * math.sin(alpha_3) / 2)
    diagonals_intersection_angle = math.asin(2 * area / (diag_1 * diag_2))

    angle_treshold = 20 * pi / 180
    if (abs(alpha_1 - pi / 2) > angle_treshold) or (abs(alpha_2 - pi / 2) > angle_treshold) or (
        abs(alpha_3 - pi / 2) > angle_treshold) or (abs(alpha_4 - pi / 2) > angle_treshold) or (
            abs(diagonals_intersection_angle - pi / 2) > angle_treshold):
        return (min_x_point[0], min_y_point[1]), (max_x_point[0], min_y_point[1]), (
            max_x_point[0], max_y_point[1]), (min_x_point[0], max_y_point[1])
    else:
...
def side_colors(image, up_left, up_right, low_right, low_left, predictor):
    ...
    p_1 = (int(up_left[0] + (1 / 6) * primary_diag[0]), int(up_left[1] + (1 / 6) * primary_diag[1]))
    p_9 = (int(up_left[0] + (5 / 6) * primary_diag[0]), int(up_left[1] + (5 / 6) * primary_diag[1]))
    p_5 = (int(up_left[0] + (1 / 2) * primary_diag[0]), int(up_left[1] + (1 / 2) * primary_diag[1]))
    p_7 = (int(low_left[0] + (1 / 6) * secondary_diag[0]), int(low_left[1] + (1 / 6) * secondary_diag[1]))
    p_3 = (int(low_left[0] + (5 / 6) * secondary_diag[0]), int(low_left[1] + (5 / 6) * secondary_diag[1]))
    ...
    colors = []
    for point in points:
        colors.append(predictor.predict_pixel(
            image[max(0, point[1]-10):min(image.shape[0], point[1]+10), 
                  max(0, point[0]-10):min(image.shape[1], point[0]+10)]
        ))
...
def main():
    ...
    if use_cnn:
        predictor = CNNPredictor("color_classification_models/cnn_color_predictor.pth",
